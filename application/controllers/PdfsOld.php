...
class Pdfs extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->cezpdf->ezText('<b>Fecha y hora de impresion:</b> ' . date('Y-m-d') ); 
        //. ', ' . date('H:i') . ' hrs.');
        $this->cezpdf->ezText('');
        $this->cezpdf->ezText('');
...
public function pdf_nomina()
    {
       ...
       $totales=$this->db->query('SELECT SUM(sueldo_bruto) as total_sb, SUM(sueldo_neto) as Total_sn  FROM percepciones  WHERE fecha_percepcion = "30-05-2017"');
        ...
        $q=$this->db->query('SELECT id_percepcion, nombre_empleado, apellido_empleado,sueldo_bruto,ars_empleado,afp_empleado,prestamo_empleado ,total_deduccion,
                      sueldo_neto, fecha_percepcion FROM empleados a, percepciones b  WHERE a.id_empleado = b.id_empleado AND fecha_percepcion = "30-05-2017"');
...
public function pdf_recibo($id_empleado)
    {
       ...
       // $this->cezpdf->ezText('');
        ...
        $q=$this->db->query('SELECT id_percepcion, id_empleado, he_empleado,comision_empleado, sueldo_bruto, ars_empleado,
         afp_empleado, prestamo_empleado, total_deduccion, fecha_percepcion, sueldo_neto FROM percepciones WHERE  fecha_percepcion LIKE  "30-05-2017" AND id_empleado LIKE ' .$id_empleado);


     $otras=$this->db->query('SELECT * FROM percepciones WHERE fecha_percepcion LIKE  "30-05-2017" AND id_empleado LIKE ' .$id_empleado);
